      ...
      <div className="lg:ml-64 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-6 max-w-6xl mx-auto">
          ...
          <div className="mb-4 flex items-center gap-4">
            ...
            <select value={activityType} onChange={handleTypeChange} className="px-3 py-2 border border-gray-300 rounded-lg">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {activities.map((activity) => (
                    <tr key={activity.id || activity.created_at}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(activity.created_at).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        {activity.employee
                          ? `${activity.employee.first_name || ''} ${activity.employee.last_name || ''}`.trim() || 'N/A'
                          : 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          ...
                          activity.type === 'email' ? 'bg-yellow-100 text-yellow-800' :
